...
(function( $ ) { 'use strict';
	$('document').ready( function( $ ) {	
		 ...
		 * Should also set tabindex=-1 so that this container can receive focus.
		 */
		...
		commentForm.submit(function(){
			...
			$( '#commentform [aria-required=true]' ).each( function() {
				...
				if ( $.trim( $(this).val() ) == '' ) {
					...
					// for accessibility, add aria-describedby with reference ID for error message.
			...
			$.ajax({
				...
				success: function( data, textStatus ){
					...
					if ( success ) {
						...
						/**
						 * *Should* set focus to error field so user can quickly tab forward into comment fields.
						 */
